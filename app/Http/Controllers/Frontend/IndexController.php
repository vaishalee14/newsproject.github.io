...
class IndexController extends Controller
{
    public function Index()
    {
        ...
        $newnewspost = NewsPost::with('category')->orderBy('id', 'DESC')->limit(8)->get();
        $newspopular = NewsPost::with('category')->orderBy('view_count', 'DESC')->limit(8)->get();


        $skip_cat_0 = Category::skip(0)->first();
        $skip_news_0 = NewsPost::where('status', 1)->where('category_id', $skip_cat_0->id)->limit(5)->get();

        $skip_cat_2 = Category::skip(2)->first();
        $skip_news_2 = NewsPost::where('status', 1)->where('category_id', $skip_cat_2->id)->limit(6)->get();
        // dd($skip_news_2);
        $skip_cat_1 = Category::skip(1)->first();
        $skip_news_1 = NewsPost::where('status', 1)->where('category_id', $skip_cat_1->id)->limit(3)->get();

        $skip_cat_4 = Category::skip(4)->first();
        $skip_news_4 = NewsPost::where('status', 1)->where('category_id', $skip_cat_4->id)->limit(5)->get();
        ...
        // return view('frontend.index', compact('newnewspost', 'newspopular', 'skip_news_0'));
    } // End Method
